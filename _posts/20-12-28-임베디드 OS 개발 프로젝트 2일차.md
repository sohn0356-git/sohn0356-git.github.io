...
code 32는 32bit가 한 명령어를 구성하는 것을 의미하고 `global은` C언어의 `extern과` 같다고 한다.
...
이제 QEMU로 읽어보자. QEMU는 ELF라는 파일 형식밖에 읽지 못한다고 한다. Object파일도 그 중 하나라고 한다.

우리가 처음 언어를 배울 때를 기억해보자.

Source Code -> Compile -> Object File -> Link -> Execute File

여기서도 동일하다. 이 Link 작업을 하기 위해 링커 스크립트라는 파일이 필요하다.
...
으... 조금만 더 버텨보자! 대충 보면 SECTIONS이라는 블록이 핵심 역할인 것 같다.
...
제일 처음에 vector_start가 들어가게 되었다. 끝으로 실행해보자. 도중에 아무리 힘들어도 결과가 좋으면 기분은 좋은 것 같다.
